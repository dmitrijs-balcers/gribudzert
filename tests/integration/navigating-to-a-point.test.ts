/**
 * A visitor asks for directions to a water point from its popup.
 */

import { waitFor, within } from '@testing-library/dom';
import { describe, expect, it, vi } from 'vitest';
import { NEAREST_TO_USER, USER, WATER_MARKER_COUNT } from '../fixtures';
import { renderApp } from '../harness';

describe('Navigating to a point', () => {
	it('opens the navigation app for the coordinates of the point', async () => {
		...
		await waitFor(() => expect(app.popupText()).toContain(`ID: ${NEAREST_TO_USER.id}`));
		...
		expect(String(url)).toContain(`${NEAREST_TO_USER.lat},${NEAREST_TO_USER.lon}`);
